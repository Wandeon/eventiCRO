...
function fakeDb(strings: TemplateStringsArray, ...values: any[]) {
  ...
  if (sql.startsWith('SELECT id, created_at, status, payload, reviewer, reviewed_at, reason FROM submissions')) {
    const status = values[0];
    const limitPlus = values[values.length - 1];
    let list = submissions.filter((s) => s.status === status);
    list.sort((a, b) =>
      a.created_at.getTime() - b.created_at.getTime() || a.id.localeCompare(b.id)
    );
    if (values.length === 4) {
      const cursorTime = new Date(values[1]);
  ...
  if (sql.startsWith("UPDATE submissions") && sql.includes("SET status = 'approved'")) {
    const reviewer = values[0];
    const id = values[1];
    const s = submissions.find((s) => s.id === id);
    if (s) {
      s.status = 'approved';
  ...
  if (sql.startsWith("UPDATE submissions") && sql.includes("SET status = 'rejected'")) {
    const reviewer = values[0];
...
class StubHono {
  ...
  async request(url: string, options: any = {}) {
    ...
    const locals: Record<string, any> = { user: { sub: 'tester' } };
    const c = {
      req: {
      ...
      body: (data: any, status = 200) => new Response(data, { status }),
    };
    if (method === 'GET') {
      return this.getHandler(c);
    }
    if (method === 'POST' && u.pathname.endsWith('/approve')) {
      return this.approveHandler(c);
    }
    if (method === 'POST' && u.pathname.endsWith('/reject')) {
...
const route = (await import('../../src/server/routes/admin/submissions.ts')).default;
