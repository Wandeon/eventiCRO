...
route.get("/", eventsRateLimit, async (c) => {
  ...
  if (q) {
    ...
    params.push(`%${q}%`);
    where.push(
      `(e.title ILIKE $${params.length - 1} OR e.description ILIKE $${params.length})`,
    );
  }
  ...
  if (category) {
    ...
    params.push(category);
    where.push(
      `(c.slug = $${params.length - 1} OR c.id::text = $${params.length})`,
    );
  }
  ...
  if (lat !== undefined && lng !== undefined && radius !== undefined) {
    ...
    where.push(
      `(6371 * acos(cos(radians($${latIdx})) * cos(radians(e.lat)) * cos(radians(e.lng) - radians($${lngIdx})) + sin(radians($${latIdx})) * sin(radians(e.lat)))) <= $${radIdx}`,
    );
  }
  ...
  if (cursor) {
    ...
    where.push(
      `(e.start_time, e.id) > ($${params.length - 1}, $${params.length})`,
    );
  }
  ...
  if (rows.length === limit) {
    ...
    next_cursor = encodeCursor(
      new Date(last.start_time).toISOString(),
      String(last.id),
    );
  }
